...
import { FACTURES_COLLECTION, FACTURES_CODE, MONTH_ARRAY, SEPARATOR} from '../models/constants';
import { getPath } from '../models/enums/eDocumentType';
import Swal from 'sweetalert2/dist/sweetalert2.js';

@Component({
  selector: 'app-gestdoc',
  templateUrl: './gestdoc.component.html',
  styleUrls: ['./gestdoc.component.scss']
})
export class GestdocComponent implements OnInit {
//Form
...
save() {
  ...
  if (this.selectedImage == null) {
    Swal.fire('Erreur', 'Veuillez choisir un document.', 'error')
  } else {
    var name = this.selectedImage.name;
    console.log("Save name : " + name);

    const filePath = this.uploadForm.value.doc_type.toLowerCase() + SEPARATOR + name;
